...
def t_class_date(request, assign_id):
    ...
    course_assign = Assign.objects.filter(id=assign_id).values('course')
    course_hours = Course.objects.filter(id=course_assign[0]['course']).values('hours')
    course_hours = int(course_hours[0]['hours']) / 2

    return render(request, 'info/t_class_date.html', {
        'att_list': att_list,
        'course_hours': course_hours,
        'forloop': len(att_list)
    })
...
def timetable(request, class_id):
    ...
    matrix = [['' for _ in range(12)] for _ in range(6)]
...
def t_timetable(request, teacher_id):
    ...
    class_matrix = [[True for _ in range(12)] for _ in range(6)]
    for i, d in enumerate(DAYS_OF_WEEK):
        t = 0
        for j in range(12):
            if j == 0:
